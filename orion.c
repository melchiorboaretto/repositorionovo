...
static void *orion(void *arg)
{
  ...
  for (int seq = 0; seq < ctx->n_pacotes; seq++) {
    ...
    sem_wait(&(ctx->empty_orion_lua)); /* decresce contador empty */
    sem_wait(&(ctx->mutex_orion_lua)); /* entra na regiao critica */
    buffer_inserir(&ctx->buf_orion_lua, &p);
    sem_post(&(ctx->mutex_orion_lua)); /* sai da regiao critica */
    sem_post(&(ctx->full_orion_lua));
...
static void *relay(void *arg)
{
  ...
  for (int i = 0; i < total_esperado; i++) {
    ...
    sem_wait(&(ctx->full_orion_lua)); /* decresce contador empty */
    sem_wait(&(ctx->mutex_orion_lua)); /* entra na regiao critica */
    buffer_remover(&ctx->buf_orion_lua, &p);
    sem_post(&(ctx->mutex_orion_lua)); /* sai da regiao critica */
    sem_post(&(ctx->empty_orion_lua));
